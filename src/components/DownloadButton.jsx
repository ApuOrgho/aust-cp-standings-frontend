...
export default function DownloadButton({
  ...
  async function handleDownload() {
    ...
    try {
      ...
      // This is the CRITICAL change: we preserve the current theme, rather than forcing light.
      clone.setAttribute("data-theme", currentTheme);

      // 4. IMPORTANT: Remove forced white background/black text, allowing CSS to control colors
      clone.style.background = ""; // Use the inherited background
      clone.style.color = ""; // Use the inherited color
      ...
      const canvas = await html2canvas(clone, {
        scale: 2,
        // Set background to null/undefined so it captures the clone's inherited background (light or dark)
